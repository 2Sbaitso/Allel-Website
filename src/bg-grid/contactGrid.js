...
import { map, lerp, getMousePos, calcWinsize, getRandomNumber } from '../other/utils';
...
export default class Grid {
    constructor(el) {
        ...
        this.items.forEach(item => this.gridItems.push(new GridContactItem(item)));
    ...
    showItems() {
        gsap
        .timeline()
        .set(this.items, {scale: 0.4, opacity: 0}, 0)
        .to(this.items, {
            duration: 2,
            ease: 'Expo.easeOut',
            scale: pos => this.gridItems[pos].invertMovement ? 0.9 : 1,
